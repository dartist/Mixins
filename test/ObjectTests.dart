...
//    raises(function() { _.keys(void 0); }, $TypeError.type, 'throws an error for `undefined` values');
  ...
  test("objects: values", () {
    equal($($({'one': 1, 'two': 2}).values()).join(', '), '1, 2', 'can extract the values from an object');
  });

  test("objects: functions", () {
    var obj = {'a': 'dash', 'b': (x) => $(x).map, 'c': new RegExp("yo"), 'd': (x) => $(x).reduce };
    deepEqual(['b', 'd'], $(obj).functions(), 'can grab the function names of any passed-in object');

//    var Animal = function(){};
//    Animal.prototype.run = function(){};
//TODO: enable when Dart supports reflection
//    equal($($(new Animal()).functions()).join(''), 'run', 'also looks up functions on the prototype');
  });
...
//  });
  ...
  test("objects: defaults", () {
    var result;
    var options = {'zero': 0, 'one': 1, 'empty': "", 'nan': double.NAN, 'string': "string"};
    ...
    ok($(options['nan']).isNaN(), "NaN isn't overridden");
  ...
  test("objects: clone", () {
    ...
    equal($(null).clone(), null, 'non objects should not be changed by clone');
    equal($(1).clone(), 1, 'non objects should not be changed by clone');
    equal($(null).clone(), null, 'non objects should not be changed by clone');
  });
...
//  });
  ...
  test("objects: isEmpty", () {
    ok(!$([1]).isEmpty(), '[1] is not empty');
    ok($([]).isEmpty(), '[] is empty');
    ok(!$({'one': 1}).isEmpty(), '{one : 1} is not empty');
    ok($({}).isEmpty(), '{} is empty');
    ok($(new RegExp('')).isEmpty(), 'objects with prototype properties are empty');
    ok($(null).isEmpty(), 'null is empty');
    ok($(null).isEmpty(), 'undefined is empty');
    ok($('').isEmpty(), 'the empty string is empty');
    ok(!$('moe').isEmpty(), 'but other strings are not');

    Map obj = {'one': 1};
    obj.remove('one');
    ok($(obj).isEmpty(), 'deleting all the keys from an object empties it');
  });
...
//    ok(_.isObject(arguments), 'the arguments object is object');
    ok($([1, 2, 3]).isObject(), 'and arrays');
//    ok(_.isObject($('html')[0]), 'and DOM element');
//    ok(_.isObject(iElement), 'even from another frame');
    ok($((){}).isObject(), 'and functions');
//    ok(_.isObject(iFunction), 'even from another frame');
    ok(!$(null).isObject(), 'but not null');
    ok(!$(null).isObject(), 'and not undefined');
    ok(!$('string').isObject(), 'and not string');
    ok(!$(12).isObject(), 'and not number');
    ok(!$(true).isObject(), 'and not boolean');
//    ok($('string').isObject(), 'but new String()'); // no equivalent in Dart
  });

  test("objects: isArray", () {
//    ok(!_.isArray(arguments), 'the arguments object is not an array');
    ok($([1, 2, 3]).isArray(), 'but arrays are');
//    ok(_.isArray(iArray), 'even from another frame');
  });

  test("objects: isString", () {
//    ok(!_.isString(document.body), 'the document body is not a string');
    ok($($([1, 2, 3]).join(', ')).isString(), 'but strings are');
//    ok(_.isString(iString), 'even from another frame');
  });

  test("objects: isNumber", () {
    ok(!$('string').isNumber(), 'a string is not a number');
//    ok(!_.isNumber(arguments), 'the arguments object is not a number');
    ok(!$(null).isNumber(), 'undefined is not a number');
    ok($(3 * 4 - 7 / 10).isNumber(), 'but numbers are');
    ok($(double.NAN).isNumber(), 'NaN *is* a number');
    ok($(double.INFINITY).isNumber(), 'Infinity is a number');
//    ok(_.isNumber(iNumber), 'even from another frame');
    ok(!$('1').isNumber(), 'numeric strings are not numbers');
  });

  test("objects: isBoolean", () {
    ok(!$(2).isBoolean(), 'a number is not a boolean');
    ok(!$("string").isBoolean(), 'a string is not a boolean');
    ok(!$("false").isBoolean(), 'the string "false" is not a boolean');
    ok(!$("true").isBoolean(), 'the string "true" is not a boolean');
//    ok(!_.isBoolean(arguments), 'the arguments object is not a boolean');
    ok(!$(null).isBoolean(), 'undefined is not a boolean');
    ok(!$(double.NAN).isBoolean(), 'NaN is not a boolean');
    ok(!$(null).isBoolean(), 'null is not a boolean');
    ok($(true).isBoolean(), 'but true is');
    ok($(false).isBoolean(), 'and so is false');
//    ok(_.isBoolean(iBoolean), 'even from another frame');
  });

  test("objects: isFunction", () {
    ok(!$([1, 2, 3]).isFunction(), 'arrays are not functions');
    ok(!$('moe').isFunction(), 'strings are not functions');
    ok($(Mixin.identity).isFunction(), 'but functions are');
//    ok(_.isFunction(iFunction), 'even from another frame');
  ...
  test("objects: isDate", () {
    ...
    ok($(new DateTime.fromMillisecondsSinceEpoch(0, isUtc: true)).isDate(), 'but dates are');
//    ok(_.isDate(iDate), 'even from another frame');
  });

  test("objects: isRegExp", () {
    ok(!$(Mixin.identity).isRegExp(), 'functions are not RegExps');
    ok($(new RegExp("identiy")).isRegExp(), 'but RegExps are');
//    ok(_.isRegExp(iRegExp), 'even from another frame');
  });

  test("objects: isFinite", () {
    ok(!$(null).isFinite(), 'undefined is not Finite');
    ok(!$(null).isFinite(), 'null is not Finite');
    ok(!$(double.NAN).isFinite(), 'NaN is not Finite');
    ok(!$(double.INFINITY).isFinite(), 'Infinity is not Finite');
    ok(!$(double.NEGATIVE_INFINITY).isFinite(), '-Infinity is not Finite');
    ok(!$('12').isFinite(), 'Strings are not numbers');
    var obj = 5;
    ok($(obj).isFinite(), 'Number instances can be finite');
    ok($(0).isFinite(), '0 is Finite');
    ok($(123).isFinite(), 'Ints are Finite');
    ok($(-12.44).isFinite(), 'Floats are Finite');
  });

  test("objects: isNaN", () {
    ok(!$(null).isNaN(), 'undefined is not NaN');
    ok(!$(null).isNaN(), 'null is not NaN');
    ok(!$(0).isNaN(), '0 is not NaN');
    ok($(double.NAN).isNaN(), 'but NaN is');
//    ok(_.isNaN(iNaN), 'even from another frame');
  });

  test("objects: isNull", () {
//    ok(!$(null).isNull(), 'undefined is not null');
    ok(!$(double.NAN).isNull(), 'NaN is not null');
    ok($(null).isNull(), 'but null is');
...
//  }
  ...
  test("objects: tap", () {
    ...
    var interceptor = (obj) { intercepted = obj; };
    var returned = $(1).tap(interceptor);
