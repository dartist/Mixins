...
//  });
  ...
  test("utility: identity", () {
    var moe = {'name': 'moe'};
    equal(Mixin.identity(moe), moe, 'moe is the same as his identity');
  });

  test("utility: uniqueId", () {
    var ids = [], i = 0;
    while(i++ < 100) ids.add(Mixin.uniqueId());
...
//
  test('result calls functions and returns primitives', () {
    ...
    strictEqual($(obj).result('x'), 'x', 'returns normal property');
    strictEqual($(obj).result('y'), 'x', 'returns value of property function');
    strictEqual($(obj).result('z'), null, 'returns null for non-existant properties');
    strictEqual($(null).result('x'), null, 'returns null when target is null');
  });
